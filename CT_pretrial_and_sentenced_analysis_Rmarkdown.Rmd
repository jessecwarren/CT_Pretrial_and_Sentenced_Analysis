...
test <- sentenced_clean_1charge %>% 
  select(identifier, sentence_days, sentenced_latest_admission_date, offense) %>% 
  group_by(identifier, sentenced_latest_admission_date) %>% 
  mutate(id = 1:n()) %>% 
  spread(id, offense)
...
merged <- merged %>% 
  ...
  mutate(offense_string_dist = stringdist(offense.x, offense.y))
...
merged_cleaned <- merged_cleaned %>% 
  filter(offense_string_dist <= 8 | is.na(offense_string_dist))
...
merged_cleaned <- merged_cleaned %>% 
  ungroup() %>% 
  select(-sentenced_facility, -pretrial_facility, -race.y, -gender.y, -special_parole_end_date) %>% 
...
#percent of people remaining
(observations_in_merged_cleaned / observations_in_merged) * 100

#vector of people in pretrial dataset in last year
pretrial_dataset_last_year_vec <- pretrial_clean_1charge %>%
  filter(latest_pretrial_download_date > most_recent_pretrial_download_date_in_dataset - 365) %>% 
  select(identifier) %>% 
  pull()
...
sentenced_clean_1charge %>%
    filter(latest_sentenced_download_date > most_recent_sentenced_download_date_in_dataset - 365) %>% 
    filter(identifier %in% pretrial_dataset_last_year_vec) %>% 
    view() 
...
merged_cleaned_cluster <- merged_cleaned %>% 
  ...
  select(sentence_days, pretrial_detention_length)
