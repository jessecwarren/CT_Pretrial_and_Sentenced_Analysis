...
CTpeople <- unique(CT_pretrial_raw[,c("identifier", "race", "gender")])
...
nrow(CTpeople) - length(unique(CT_pretrial_raw[,"identifier"])) 

#exclude the people coded with different race and/or genders across the time period.
CTpeople$dup <- duplicated(CTpeople$identifier)
CTpeopledup <- subset(CTpeople, dup == "TRUE")
CTpeopledup <- CTpeopledup[,1]
CTp <- CTpeople %>% 
  filter(!identifier %in% CTpeopledup)
CTp <- CTp[,c(1:3)]
...
ggsave("Graphs/age_race-05-02-20.png", width=8, height=5, dpi=900)
```
